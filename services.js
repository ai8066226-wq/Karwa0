...
onAuthStateChanged(auth, user => {
  ...
  state.userUnsubscribe = onSnapshot(doc(db, "users", user.uid), snapshot => {
    ...
    if (state.userData.role === "serviceApplicant" || state.userData.role === "serviceProvider") {
      ...
      byId("deniedMessage").textContent = "هذا حساب عميل ولا يمكن استخدامه في بوابة الخدمات الأخرى.";
      showView("denied");
    } else if (state.userData.role === "driver" || state.userData.role === "driverApplicant") {
      byId("deniedMessage").textContent = "هذا حساب كابتن ولا يمكن استخدامه في بوابة الخدمات الأخرى.";
      showView("denied");
    } else {
