# Karwa0 — Phase 49: تسعير ديناميكي، شحن محلي، دعوات وتحسين الخريطة

## التعديلات في هذه النسخة

- خيارات التكسي أصبحت تتحدث حسب **المسافة ووقت المسار وتقدير الازدحام**، وتعرض السعر ووقت الرحلة المتوقع لكل فئة بدل الأسعار الثابتة فقط. تقدير الازدحام محلي مشتق من زمن المسار مقارنة بسرعة انسيابية، وليس بيانات ازدحام لحظية من مزود مدفوع.
- حُذف اختيار **طريقة الدفع** من واجهة حجز العميل.
- أزيل زر الشحن التجريبي، وأضيف تدفق **شحن محلي عبر Mastercard**: يبدأ رصيد الحساب الجديد من صفر، والعميل يحول المبلغ ويدخل المبلغ ومرجع العملية، ثم تعتمد الإدارة الطلب ليضاف نفس المبلغ إلى رصيده مرة واحدة.
- أضيفت أكواد دعوة شخصية ومشاركة من الجهاز. كود الدعوة يعمل كخصم أول مرة، ونسبة الخصم وسقفه يحددهما المدير.
- لوحة الإدارة تحتوي إعدادات عمولة منفصلة للتكسي، توصيل الأغراض، الطعام، توصيل الخدمات، وكذلك فئات المطاعم والبقالة والتسوق والصيانة والمنزل والصحة وغيرها.
- عولج ثقل اختيار الموقع: العلامة تتحرك فورًا، وتم تهدئة حساب المسار، وفصل طلبات العنوان العكسي، وإهمال النتائج القديمة بدل تكديس طلبات الشبكة.
- قواعد Firestore تمنع العميل من زيادة رصيده بنفسه، وتحصر اعتماد الشحن وتغيير الرصيد بالإدارة.

## مهم قبل النشر

1. انشر `firestore.rules` الجديدة قبل اختبار الشحن والدعوات.
2. من لوحة الإدارة افتح قسم **التسعير والعمولات والشحن** واضبط معرف التحويل المحلي واسم المستلم والنسب المطلوبة. لا تحفظ CVV أو PIN أو كلمات مرور بطاقات داخل Firestore.
3. ارفع ملفات الواجهة كاملة، لأن Service Worker ارتفع إلى Phase 49 لتجنب بقاء نسخة PWA قديمة.
4. الشحن الحالي **مراجعة محلية بإيصال/مرجع تحويل** وليس خصمًا آليًا من Mastercard. للخصم الآلي يلزم مزود دفع مرخص وAPI/merchant credentials.
